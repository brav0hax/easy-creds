#!/bin/bash
# installer easy-creds v3.7
# Last updated 10/28/2012 - J0hnnyBrav0
...
f_debian(){
	...
	for i in $reqs; do
		...
		if [ ! -e /usr/bin/$i ] && [ ! -e /usr/sbin/$i ] && [ ! -e /usr/local/sbin/$i ] && [ ! -e /usr/local/bin/$i ] && [ -z "$isinstalled" ]; then
				echo -e "\e[1;33m[-] $i is not installed, will attempt to install...\e[0m"
...
if [ ! -e /usr/bin/aircrack-ng ] && [ ! -e /usr/sbin/aircrack-ng ] && [ ! -e /usr/local/sbin/aircrack-ng ] && [ ! -e /usr/local/bin/aircrack-ng ]; then
	update=1
	echo -e "\n\e[1;33m[*] Downloading and installing aircrack-ng from SVN\e[0m"
	...
	cd $path
	echo -e "\n\e[1;32m[+] Aircrack-ng has been installed...\e[0m"
	sleep 5
fi
...
f_ettercapinstall(){
etterupdate=
ettercap -V > /tmp/ec-install/etter 2>&1
oldetter=$(cat /tmp/ec-install/etter | grep -o "0.7.3")

if [ ! -z "$oldetter" ]; then
	etterupdate=1	
	update=1
	echo -e "\e[1;33m[-] Your ettercap version is ancient will install latest version...\e[0m"
	sleep 2
	echo -e "\e[1;33m[-] Getting ettercap prerequisites, this will take some time...\e[0m"
	sleep 2
	if [ "$mainchoice" == "1" ]; then
		apt-get -y install libpcap0.8 libpcap0.8-dev libnet1 libnet1-dev libnet6-1.3-0 libnet6-1.3-dev libpthread-stubs0 libpthread-stubs0-dev zlibc libtool automake flex bison libncurses5 libncurses5-dev libgtk2.0-dev libgtk2.0-0 libpcre3-dev libpcre3 > /dev/null 2>&1
		...
		cd /tmp/ec-install
		tar -zxvf ettercap-0.7.4.1.tar.gz &> /dev/null
		cd ettercap
		./autogen.sh
		./configure && make && make install &> /dev/null
		cd $path
		echo -e "\n\e[1;32m[+] ettercap has been installed...\e[0m"
		sleep 5
	else
...
if [ ! -e /usr/bin/ettercap ] && [ ! -e /usr/local/bin/ettercap ] && [ ! -e /usr/sbin/ettercap ] && [ ! -e /usr/local/sbin/ettercap ]; then
	etterupdate=1	
	update=1
	echo -e "\e[1;33m[-] ettercap is not installed, will attempt to install...\e[0m"
	sleep 2
	echo -e "\e[1;33m[-] Getting ettercap prerequisites...\e[0m"
	sleep 2
	if [ "$mainchoice" == "1" ]; then
		apt-get -y install libpcap0.8 libpcap0.8-dev libnet1 libnet1-dev libnet6-1.3-0 libnet6-1.3-dev libpthread-stubs0 libpthread-stubs0-dev zlibc libtool automake flex bison libncurses5 libncurses5-dev libgtk2.0-dev libgtk2.0-0 libpcre3-dev libpcre3 > /dev/null 2>&1
		echo -e "\n\e[1;33m[*] Downloading and installing ettercap from source...\e[0m"
...
if [ -z $etterupdate ]; then
	echo -e "\e[1;32m[+] I found ettercap installed on your system\e[0m"
	sleep 2
fi
...
if [ ! -e /usr/bin/radiusd ] && [ ! -e /usr/sbin/radiusd ] && [ ! -e /usr/local/sbin/radiusd ] && [ ! -e /usr/local/bin/radiusd ]; then
	update=1
	echo -e "\e[1;33m[-] free-radius is not installed, will attempt to install...\e[0m"
	sleep 2
	echo -e "\n\e[1;33m[*] Downloading freeradius server 2.1.11 and the wpe patch...\e[0m"
	...
	echo -e "\n\e[1;33m[*] Installing the patched freeradius server...\e[0m"
	sleep 3
	./configure && make && make install &> /dev/null
	cd /usr/local/etc/raddb/certs/
	./bootstrap &> /dev/null
	cd $path
	echo -e "\n\e[1;32m[+] The patched freeradius server has been installed\e[0m"
	sleep 5
else
	echo -e "\e[1;32m[+] I found free-radius installed on your system\e[0m"
	sleep 2
fi
}
...
if [ ! -e /usr/bin/asleap ] && [ ! -e /usr/sbin/asleap ] && [ ! -e /usr/local/sbin/asleap ] && [ ! -e /usr/local/bin/asleap ]; then
	...
	cd $path
	echo -e "\n\e[1;32m[+] asleap has been installed...\e[0m"
	sleep 5
else
	echo -e "\e[1;32m[+] I found asleap installed on your system\e[0m"
	sleep 2
fi
}
...
if [ ! -e /usr/bin/ferret ] && [ ! -e /usr/sbin/ferret ] && [ ! -e /usr/local/sbin/ferret ] && [ ! -e /usr/local/bin/ferret ]; then
	update=1
	echo -e "\e[1;33m[-] ferret is not installed, will attempt to install...\e[0m"
	sleep 2
	echo -e "\n\e[1;33m[*] Downloading and installing ferret from SVN\e[0m"
	...
	cd $path
	echo -e "\n\e[1;32m[+] ferret has been installed...\e[0m"
	sleep 5
else
	echo -e "\e[1;32m[+] I found ferret installed on your system\e[0m"
	sleep 2
fi
...
if [ ! -e /usr/bin/hamster ] && [ ! -e /usr/sbin/hamster ] && [ ! -e /usr/local/sbin/hamster ] && [ ! -e /usr/local/bin/hamster ] && [ -z $(locate hamster.js) ]; then
	update=1
	echo -e "\e[1;33m[-] hamster is not installed, will attempt to install...\e[0m"
	sleep 2
	echo -e "\n\e[1;33m[*] Downloading and installing hamster from source...\e[0m"
	cd /tmp/ec-install
	...
	cd $path
	echo -e "\n\e[1;32m[+] hamster has been installed...\e[0m"
	sleep 5
else
	echo -e "\e[1;32m[+] I found hamster installed on your system\e[0m"
	sleep 2
fi
...
if [ ! -e /usr/bin/msfconsole ] && [ ! -e /usr/sbin/msfconsole ] && [ ! -e /usr/local/sbin/msfconsole ] && [ ! -e /usr/local/bin/msfconsole ]; then
	update=1
	echo -e "\n\e[1;33m[*] Downloading Metasploit from www.metasploit.com, this will take a while to complete\e[0m"
	...
	sleep 2
	echo -e "\n\e[1;33m[*] Updating the Metasploit framework \e[0m"
	sleep 2
	msfupdate
else
...
if [ ! -e /usr/bin/sslstrip ] && [ ! -e /usr/sbin/sslstrip ] && [ ! -e /usr/local/sbin/sslstrip ] && [ ! -e /usr/local/bin/sslstrip ] && [[ -z $(locate sslstrip.py) ]]; then
	...
	cd $path
	echo -e "\n\e[1;32m[+] SSLStrip has been installed...\e[0m"
	sleep 5
else
	echo -e "\e[1;32m[+] I found sslstrip installed on your system\e[0m"
	sleep 2
fi
...
f_install(){
	...
	cd ..
	path=$PWD
	if [ $PWD/easy-creds == $easycredpath/easy-creds ]; then 
		echo "Can't install into the folder..... from the folder.  Choose a different path.  :P"
		unset easycredpath
		sleep 5
		f_mainmenu
	fi
	rm -rf $easycredpath/easy-creds > /dev/null
